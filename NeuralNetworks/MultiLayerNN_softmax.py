...
class NeuralNetwork(object):
    ...
    def build_model(self, X, y, hidden_layers, n_inputs, n_outputs, print_loss=True):
        ...
        # b1 = np.random.uniform(size=(1, hidden_layers[0]), low=-1, high=-1)
        # print W1
        weights[0] = W1
        bias[0] = b1
        for h in range(num_hls-1):
            ...
            # w = np.random.uniform(size=(hidden_layers[h], hidden_layers[h + 1]))
            # b = np.random.uniform(size=(1, hidden_layers[h + 1]))
            weights[h+1] = w
            bias[h+1] = b
        W_last = np.random.randn(hidden_layers[num_hls - 1], n_outputs) / np.sqrt(hidden_layers[num_hls - 1])
        b_last = np.zeros((1, n_outputs))
        # W_last = np.random.uniform(size=(hidden_layers[num_hls - 1], n_outputs))
        # b_last = np.random.uniform(size=(1, n_outputs))
