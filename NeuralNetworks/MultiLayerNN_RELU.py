...
class NeuralNetwork(object):
    ...
    def build_model(self, X, y, hidden_layers, n_inputs, n_outputs, print_loss=True):
        ...
        # W1 = np.random.uniform(size=(n_inputs, hidden_layers[0]), low=-1, high=1)
        # b1 = np.random.uniform(size=(1, hidden_layers[0]), low=0, high=1)
        # print W1
        weights[0] = W1
        bias[0] = b1
        for h in range(num_hls-1):
            ...
            # w = np.random.uniform(size=(hidden_layers[h], hidden_layers[h + 1]), low=-1, high=1)
            # b = np.random.uniform(size=(1, hidden_layers[h + 1]), low=0, high=1)
            weights[h+1] = w
            bias[h+1] = b
        W_last = np.random.randn(hidden_layers[num_hls - 1], n_outputs) / np.sqrt(hidden_layers[num_hls - 1])
        b_last = np.zeros((1, n_outputs))
        # W_last = np.random.uniform(size=(hidden_layers[num_hls - 1], n_outputs), low=-1, high=1)
        # b_last = np.random.uniform(size=(1, n_outputs), low=0, high=1)
        ...
        for e in xrange(0, self.max_iters):
            ...
            for idx in range(count_iters):
                if idx == count_iters-1:
                    # print "Layer: ", idx, " softmax here"
                    # z = self.softmax(np.dot(layer_input, weights[idx]) + bias[idx])
                    z = self.sigmoid(np.dot(layer_input, weights[idx]) + bias[idx])
                else:
                    z = self.relu(np.dot(layer_input, weights[idx]) + bias[idx])
            ...
            for lyr in range(count_iters-1, 0, -1):
                # print "lyr: ", lyr
                if lyr == count_iters-1:
                    # print "Layer: ", lyr, " softmax derivative here"
                    layer_slope = self.derivatives_sigmoid(right_lyr)
                    # layer_slope = self.derivatives_softmax(right_lyr)
                else:
                    layer_slope = self.derivatives_relu(right_lyr)
            ...
            layer_slope = self.derivatives_relu(a[0])
            d_output = np.multiply(error_at_hl, layer_slope)
            delta_op.append(d_output)
    ...
    def predict(self, X_test):
        ...
        for idx in range(count_iters):
            if idx == count_iters - 1:
                ...
                # z = self.softmax(np.dot(layer_input, wts[idx]) + bs[idx])
            else:
                z = self.relu(np.dot(layer_input, wts[idx]) + bs[idx])
                # a[idx] = logistic.cdf(z[idx])
            a.append(z)
            layer_input = z
