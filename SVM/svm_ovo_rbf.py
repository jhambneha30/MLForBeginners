...
def decision_function(test_point, clf):
    ...
    for sv_index in range(sv_length):
        sv_values[sv_index] = np.exp(-gamma * ((np.linalg.norm(support_vectors[sv_index] - [test_point])) ** 2))
...
# Finding the label with max number of votes
